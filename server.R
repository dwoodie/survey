...
shinyServer(function(input, output) {
  ...
  output$interest_plot <- renderPlot({
    ...
    interest_temp <- survey_data() %>% 
      separate_rows(Interests, sep = ", ") %>% 
      mutate(Interests = trimws(Interests)) %>% 
      group_by(Interests) %>% 
      summarise(Responses = length(Interests)) %>% 
      arrange(desc(Responses))

    interest_temp$Interests = factor(interest_temp$Interests,
                                     levels = interest_temp$Interests[order(interest_temp$Responses, decreasing = TRUE)])
    
    interest_temp <- interest_temp[!is.na(interest_temp$Interests),]
    
    p <- ggplot(interest_temp, aes(x = Interests, y = Responses)) + 
      geom_bar(colour="#0a2040", fill="#0a2040", stat = "identity") +
